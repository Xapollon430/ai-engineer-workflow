...
- `skills/skill-to-issues/SKILL.md` — adapted: removed the dependency on `setup-matt-pocock-skills`, dropped the GitHub issue-tracker integration in favor of writing issue files directly to `docs/issues/<NN>-<slug>.md` in dependency order, added a "find the source" step that picks up the latest PRD or spec from this plugin's other skills. Skill renamed to `skill-to-issues`.
